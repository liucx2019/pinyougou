...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	private void saveToRedis() {
		...
		for (TbTypeTemplate typeTemplate : list) {
			...
			redisTemplate.boundHashOps("brandList").put(typeTemplate.getId(), brandList);// key=模板id,value=品牌列表ids

			// 存储规格列表
			List<Map> specList = findSpecList(typeTemplate.getId());// 根据模板 ID 查询规格列表
			redisTemplate.boundHashOps("specList").put(typeTemplate.getId(), specList);// key=模板id,value=规格列表ids
